...
def _guess_title_from_obj(obj: dict, filename: str, kind: str = "") -> str:
    """Best-effort human title from common fields; falls back to filename."""
    ...
    keys_by_kind = {
        "service": [
            "title","service_name","name","headline","service","offering","product_name",
            "practice_area","practice_area_name","label","category","subtype","type"
        ],
        "award": [
            "title","award_name","name","certification_name","certification","license",
            "accreditation","badge","designation","honor","recognition"
        ],
    }
    keys = keys_by_kind.get(kind, ["title","name","headline","label"])

    candidate = _first_nonempty(*(obj.get(k) for k in keys))
    if _is_placeholder_title(candidate):
        # If there is a nested object (rare), try a second pass
        for k in ("award","service","item"):
            if isinstance(obj.get(k), dict):
                candidate = _first_nonempty(*(obj[k].get(x) for x in keys))
...
def generate_services_page():
    ...
    def _guess_title(obj, filename):
        # Use content-driven titles whenever possible (prevents Services1/Awards1 headings)
        return _guess_title_from_obj(obj if isinstance(obj, dict) else {}, filename, kind="service")
...
def generate_awards_page():
    ...
    print(f"🔍 Checking awards data in: {awards_dir}")
    if not os.path.exists(awards_dir):
        # Graceful placeholder page (prevents workflow failures)
        with open("awards.html", "w", encoding="utf-8") as f:
            f.write(generate_page("Awards", "<p>No awards found yet.</p>"))
        print("✅ awards.html generated (placeholder)")
        return True
    ...
    for file in sorted(os.listdir(awards_dir)):
        ...
        for rec in records:
            if not isinstance(rec, dict):
                continue
            title = _guess_title_from_obj(rec, filepath, kind="award")
            desc = _guess_description(rec) or _first_nonempty(rec.get("details"), rec.get("summary"))
            items.append(f"""
            <div class="card">
                <h2>{escape_html(title)}</h2>
                {'<p>' + escape_html(desc) + '</p>' if desc else ''}
            </div>
            """)
    ...
    with open("awards.html", "w", encoding="utf-8") as f:
        f.write(generate_page("Awards", "".join(items)))

    print(f"✅ awards.html generated ({len(items)} items from {files_processed} file(s))")
    return True
...
if __name__ == "__main__":
    ...
    html_files = ["index.html", "about.html", "services.html", "testimonials.html", "faqs.html", "help.html", "awards.html", "contact.html"]
    for f in html_files:
        if os.path.exists(f):
            os.remove(f)
